...
mutex = Lock()
k_custom_invalid_depth_value = 0
k_custom_offset = 0
...
class HandleNanAndNegativeInDepth(object):
    def __init__(self):
    ...
    def acquire_profile_data_using_callback(self) -> bool:
        ...
        self.callback = CustomAcquisitionCallback(self.data_width)
    ...
    def handle_values_and_save_depth_map(self, depth_file_name):
        depth = cv2.Mat(self.profile_batch.get_depth_map().data())
        valid_depth_mask = cv2.Mat(
            np.zeros((self.profile_batch.height(), self.profile_batch.width()), np.uint8))
        nan_mask = cv2.Mat(
            np.zeros((self.profile_batch.height(), self.profile_batch.width()), np.uint8))
        cv2.compare(depth, depth, cv2.CMP_EQ, valid_depth_mask)
        cv2.bitwise_not(valid_depth_mask, nan_mask)
        min_depth_value, _, _, _ = cv2.minMaxLoc(depth, valid_depth_mask)
        if min_depth_value < 0:
            if k_custom_offset == 0:
                depth = depth + (-min_depth_value)
            else:
                depth = depth + k_custom_offset
        depth[nan_mask > 0] = k_custom_invalid_depth_value
        cv2.imwrite(depth_file_name, depth)

    def save_intensity(self, intensity_file_name):
    ...
    def main(self):
        ...
        self.handle_values_and_save_depth_map(
            "depth_without_nan_and_negative.tiff")
        self.save_intensity("intensity.png")
